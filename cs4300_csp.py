...
def solve_backtracking_mrv(csp: CSP) -> Iterable[Assignment]:
    ...
    def backtrack(assignment: Assignment):
        ...
        for val in list(domains[v]):
            ...
            if consistent_with_local(v, assignment):
                ...
                for w in unassigned_vars:
                    ...
                    for vv in list(domains[w]):
                        ...
                        if not consistent_with_local(w, assignment):
                            domains[w].remove(vv)
                            removed_from_w.append(vv)
                        del assignment[w]
                    
                    if removed_from_w:
                        pruned.append((w, removed_from_w))
                ...
                if ok:
                    yield from backtrack(dict(assignment))
